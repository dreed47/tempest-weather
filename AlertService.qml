...
Item {
  ...
  // api.weather.gov/points, or the alertRadarSite override. Both are
  // whitelisted (Model.isValidRadarStation) before use -- this value gets
  // spliced straight into a radar.weather.gov URL.
  readonly property string configuredRadarSiteRaw: String(cval("alertRadarSite", "")).replace(/^\s+|\s+$/g, "").toUpperCase()
  readonly property string configuredRadarSite: Model.isValidRadarStation(configuredRadarSiteRaw) ? configuredRadarSiteRaw : ""
    ...
    stdout: StdioCollector {
      ...
      onStreamFinished: {
        try {
          ...
          var rs = d && d.properties ? String(d.properties.radarStation || "").toUpperCase() : ""
          if (Model.isValidRadarStation(rs)) root.resolvedRadarStation = rs
        } catch (e) {}
      }
    }
  }
